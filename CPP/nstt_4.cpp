...
//    }
    ...
    TreapNode &operator=(TreapNode other) noexcept {
        std::swap(this->value, other.value);
        if (other.left) {
            this->left = new TreapNode(*other.left);
        }
...
TEST(move_constructor_and_assignment_operator_test_suite, move_test) {
    TreapNode *treap = get_test_data();
    TreapNode other_treap = *treap;
    TreapNode third_treap = std::move(other_treap);
    EXPECT_TRUE(*treap == third_treap);
    EXPECT_FALSE(*treap == other_treap);
    other_treap = std::move(*treap);
    EXPECT_TRUE(third_treap == other_treap);
    EXPECT_FALSE(third_treap == *treap);
    delete treap;
