...
class Treap {
    TreapNode *root;
public:
    Treap(const std::vector<TreapVal> &values) {
        this->root = (new TreapNode(values))->getNoParents();
    }

    Treap(Treap &other) {
        this->root = new TreapNode(*other.root);
    }
    ...
    void insert(const TreapVal &value) { root = root->insert(value); }
...
Treap *get_test_data() {
    ...
    auto *treap = (new Treap(values));
    return treap;
}

TEST(alredy_not_lonley_test_suite, linear_create_test) {
    Treap *treap = get_test_data();
...
TEST(copy_and_copy_assignment_test_suite, copy_test) {
    Treap *treap = get_test_data();
    Treap *new_treap = (new Treap(*treap));
    EXPECT_TRUE(*treap == *new_treap);
    Treap other_treap = *treap;
