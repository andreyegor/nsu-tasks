...
class Bloom:
    ...
    def __init__(self, quan, prob):
        length = (log(prob, 2)*quan)/log(1/2)
        
        
        self.__hshs = ceil(-log(prob, 2))  # ???
        self.__bitset = [False] * ceil(self.__hshs*quan/log(2))
        
        self.__abhshs = [[randint(0, len(self.__bitset)-1), randint(0, len(self.__bitset)-1)] for i in range(self.__hshs)]
...
if __name__=='__main__':
    TEST_RANGE = 5000
    PROB = 0.001
    test = sample(range(0,TEST_RANGE*20), TEST_RANGE*2)
    ...
    for e in not_includes:
        if blm.lookup(e):
            cntr+=1
    assert cntr/TEST_RANGE<=PROB*1.5
