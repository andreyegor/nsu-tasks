"""
Реализовать три алгоритма умножения квадратных матриц:
классический, через 8 рекурсивных вызовов и алгоритм
Штрассена.

Кроме того, реализовать тестовый стенд, принимающий на
вход набор алгоритмов и набор входных данных, а
возвращающий подробный отчет, сколько времени заняла
работа каждого алгоритма на каждом примере входных
данных.

Данные записывать в отформатированную таблицу
(см. 9 мини задачу из курса Python)

Продемонстрировать с помощью стенда, начиная с какого
порядка данных Штрассен начинает выигрывать

Замечание:

Запускать каждый алгоритм на одних и тех же данных
стоит по несколько раз. По результатам этих запусков
необходимо находить выборочное среднее, стандартное
отклонение и среднее геометрическое, и выводить именно
их комбинацию в качестве результата.

Это повысит правдоподобность результатов, снизив
влияние внешних факторов на ваше измерение.
"""
